...
//            String packetBodyContent = "{\"userName\": \"" + "alamgir" + "\", \"password\": \"" + "pass" + "\"}";
            ...
            ds.send(new DatagramPacket(sendPacket, sendPacket.length, InetAddress.getLocalHost(), CLIENT_PORT));
    ...
    public void run() {
        while (running) {
            try {
                ...
                send(packetHeader, "{\"fistName\": \"Alagir\", \"lastName\": \"Kabir\", \"userName\": \"alamgir\", \"password\": \"pass\"}");
            } catch (IOException ex) {
                Logger.getLogger(DatagramClient.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
